...
create table if not exists purchased_by(date_of_purchase date, customer_id integer, item_id integer, primary key(date_of_purchase,customer_id,item_id), foreign key(customer_id) references customer(person_id), foreign key(item_id) references item(item_id));
