...
/*create table if not exists book(isbn primary key not null, item_id integer not null, foreign key(item_id) references item(item_id), edition text);*/
...
create table if not exists person(person_id integer primary key, first_name text, last_name text, city_id integer, foreign key(city_id) references city(city_id));
